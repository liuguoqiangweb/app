...
export default {
  ...
  methods: {
    doDaka () {
      this.$http.post('/amoy/home/dk-do', {id: this.dakaInfo.info.id || 0}, true).then(resdaka => {
        if (resdaka.code === 0) {
    ...
    getInfo () {
      ...
      this.$http.post('/amoy/home/da-ka', {}, true).then(res => {
        if (res.code === 0) {
          ...
          console.log('/amoy/home/da-ka>res' + JSON.stringify(res))
          this.numbers = data.numbers
          this.partNumber = data.numbers[0][0]
          this.members = data.members
          this.manyNumber = Number(data.money * data.price).toFixed(2) // 修改打卡页面挑战成功后，中间的按钮显示的数字尾数太长
          ...
          if (That.checkNowBetweenTimeQuantum(data.startTime, data.endTime)) {
            // 打卡时间
            if (data.info) {
              // 已报名
              // 进行打卡
    ...
    * */
    computeBounsTime () {
      let time = parseInt(this.endTime.split(':')[0]) + 2
      if (time > 24) {
        return time - 24
      } else {
        return time
    ...
    // 检查当前时间是否在一个时间段中
    checkNowBetweenTimeQuantum (startTime, endTime) {
...
//         }
      ...
      console.log('读秒：' + this.staetEndTime)
    },
    // 邀请好友弹窗
    isInvitation () {
