  ...
  <div id="materials">
      ...
      <div class="outBox">
        ...
        <div class="ulbox">
          <ul>
            ...
            <!--<p>sdf</p>-->
            <!--</li>-->
            <li @click="articleDownOne(4)">
              <img src="../../assets/img/share/one.png" alt />
              <p>保存图片</p>
            </li>
        ...
        <div @click="show3=false" class="cancle">取消</div>
      </div>
    </div>
      ...
      <div class="outBox">
        ...
        <div class="ulbox">
          <ul>
            ...
            <!--<p>sdf</p>-->
            <!--</li>-->
            <li @click="imgDown(0)" v-if="imgArr.length === 1">
              <img src="../../assets/img/share/one.png" alt />
              <p>保存图片</p>
            </li>
            <li v-else @click="imgDown(0)">
              <img src="../../assets/img/share/plct.png" alt />
              <p>批量存图</p>
            </li>
          </ul>
        </div>
        <div @click="show2=false" class="cancle">取消</div>
      </div>
    </div>
    <div class="shareArea" v-show="show1" v-transfer-dom>
      <div class="outBox">
        ...
        <div @click="show1=false" class="cancle">取消</div>
      </div>
    </div>
      ...
      <div class="outBox">
        ...
        <div class="ulbox">
          <ul>
            ...
            <li @click="imgDownNew(0)" v-if="imgArr.length === 1">
              <img src="../../assets/img/share/one.png" alt />
              <p>保存图片</p>
            </li>
            <li v-else @click="imgDownNew(0)">
              <img src="../../assets/img/share/plct.png" alt />
              <p>批量存图</p>
            </li>
          </ul>
        </div>
        <div @click="show0=false" class="cancle">取消</div>
      </div>
    </div>
    <!-- 之前的使用了imgShareNew等的new方法，改成上面的统一走imgShare等方法 -->
    <!--<div class="shareArea" v-show="show0" v-transfer-dom>-->
    <!--<div class="outBox">-->
    <!--<p class="aim">分享到</p>-->
    <!--<div class="ulbox">-->
    ...
    <mescroll-vue ref="mescroll" :down="mescrollDown" :up="mescrollUp" @init="mescrollInit">
      ...
      <div id="store" v-if="cate && cate[cateIndex].sonList && cate[cateIndex].sonList.length > 0">
        <ul>
          ...
          <li
            v-for="(second, index) of cate[cateIndex].sonList"
            :class="{selected: secondIndex === second.id}"
            :key="index"
            @click="changeCate(second.id)"
          >{{second.title}}</li>
        </ul>
      </div>
      <template v-for="(item, index) of list">
        <div
          class="recommendBox"
          :key="index"
          v-if="(item.type === '0' || item.type === '1') && (item.items.length === 3 || item.items.length === 5 || item.items.length === 6 || item.items.length === 7 || item.items.length === 8 || item.items.length === 9)"
        >
          <div class="headLeft">
          ...
          <div class="contentRight">
            <p class="userName">
              <span>
                {{item.nickname}}
                <img
                  src="../../assets/img/super/tb1.png"
                  alt
                  v-if="item.items[0].from_id === '11'"
                />
                <img
                  src="../../assets/img/super/tm.png"
                  alt
                  v-else-if="item.items[0].from_id === '12'"
                />
                <img
                  src="../../assets/img/super/jd.png"
                  alt
                  v-else-if="item.items[0].from_id === '21'"
                />
                <img
                  src="../../assets/img/super/pdd.png"
                  alt
                  v-else-if="item.items[0].from_id === '31'"
                />
              </span>
              <span class="share" @click="share(item)">
                <img src="../../assets/img/source/share.png" alt />
                {{item.share_num}}
              </span>
            </p>
            <p class="time">{{item.created_at*1000 | dateFormat}}</p>
            <p class="earn" v-if="item.type === '0' && item.estimated_earn > 0">
              <img src="../../assets/img/source/money.png" alt />
              预估赚￥{{item.estimated_earn}}
            </p>
            ...
            <div class="imgBox">
              <ul>
                <li v-for="(imgs,i) of item.items" :key="i" @click="detail(imgs)">
            ...
            <div
              class="copyBox"
              v-if="item.type === '0' && (item.items[0].from_id === '11' || item.items[0].from_id === '12')&& item.items[0].zk_final_price"
            >
              <p>{{item.items[0].title}}&nbsp;&nbsp;&nbsp;&nbsp;原价：{{item.items[0].zk_final_price}}券后：{{item.items[0].coupon_price}}，复制本条消息{{item.ambush}}打开【手机TAO宝】</p>
              <div @click="getWord(item)">
                复制
                <br />口令
              </div>
            </div>
        ...
        <div
          class="recommendBox conductBox"
          :key="index"
          v-else-if="(item.type === '0' || item.type === '1') && (item.items.length === 1 || item.items.length === 2 || item.items.length === 4)"
        >
          <div class="headLeft">
            <img :src="item.head_img" alt />
          </div>
          <div class="contentRight">
            ...
            <div class="text" v-html="item.title" @click="copyTitle(item.title)"></div>
            <div class="imgBox">
              <ul>
                <li v-for="(imgs,i) of item.items" :key="i" @click="detail(imgs)">
                  <img v-lazy="imgs.img_url" alt :key="imgs.img_url" />
                </li>
              </ul>
            </div>
            <div
        ...
        <div
          class="recommendBox"
          :key="index"
          v-if="(item.type === '2')  && (item.items.length === 3 || item.items.length === 5 || item.items.length === 6 || item.items.length === 7 || item.items.length === 8 || item.items.length === 9)"
        >
          <div class="headLeft">
            <img :src="item.head_img" alt />
          </div>
          <div class="contentRight">
            ...
            <div class="imgBox">
              <ul>
                <li v-for="(imgs,i) of item.items" :key="i">
                  <img v-lazy="imgs.img_url" alt :key="imgs.img_url" />
                </li>
              </ul>
            </div>
        ...
        <div
          class="recommendBox conductBox"
          :key="index"
          v-else-if="(item.type === '2')  && (item.items.length === 1 || item.items.length === 2 || item.items.length === 4)"
        >
          <div class="headLeft">
            <img :src="item.head_img" alt />
          </div>
          <div class="contentRight">
            ...
            <div class="imgBox">
              <ul>
                <li v-for="(imgs,i) of item.items" :key="i">
                  <vue-preview :slides="imgs.img_url | urlArrFormat" @close="handleClose">
                    <!-- <img v-lazy="imgs.img_url" alt :key="imgs.img_url" />  -->
                   </vue-preview> 
                </li>
              </ul>
        ...
        <div
          class="recommendBox schoolBox"
          :key="index"
          v-else-if="(item.type === '3')  && item.video_img !== ''"
        >
          <div class="headLeft">
            <img :src="item.head_img" alt />
          </div>
          <div class="contentRight">
            <div class="topBox">
              <p class="userName">
                <span>{{item.nickname}}</span>
                <span class="share" @click.stop="share(item)">
                  <img src="../../assets/img/source/share.png" alt />
                  {{item.share_num}}
                </span>
              </p>
            ...
            <p class="go2" @click="article(item)">
              <span>查看文章</span>
              <img src="../../assets/img/source/go.png" alt />
            </p>
        ...
        <div class="update">
          ...
          <div class="contentBox">
            ...
            <div class="update_btn">
              <span @click="showToast1 = false">稍后再说</span>
              <span @click="oauth">马上认证</span>
            </div>
          </div>
        </div>
      </x-dialog>
...
export default {
  ...
  computed: {
    // tag: 'admin'-后台手动上传的发圈素材   'api'-返回的是好货专场的接口的数据（每张图对应的商品id不一样） 'single'-返回的是每日精选的接口的数据（每张图对应的商品id一样）
    tag() {
  ...
  filters: {
    dateFormat,
    urlArrFormat:function(urldata){
      let urlArr=[]
      urlArr.push({src:urldata,msrc:urldata,w:1000,h:1000})
      return urlArr
    }
  },
  components: {
  ...
  methods: {
    ...
    handleScroll(mescroll, y, isUp) {
      let that = this;
      that.opacity =
        y / that.$refs.mate.offsetHeight > 1
          ? 1
          : y / that.$refs.mate.offsetHeight;
          ...
          .then(res => {
            if (res.code === 0) {
              if (res.data.rid === "" && res.data.oauth_url !== "") {
                ...
                let img =
                  this.link +
                  "/amoy/user/create-share?num_iid=" +
                  this.item.items[i].goods_id +
                  "&item_url=" +
                  res.data.coupon_url +
                  "&type=" +
                  this.item.items[i].from_id +
                  "&title=" +
                  encodeURI(res.data.title) +
                  "&thumb=" +
                  this.item.items[i].img_url +
                  "&coupon_money=" +
                  res.data.coupon_money +
                  "&coupon_price=" +
                  res.data.coupon_price +
                  "&origin_price=" +
                  res.data.zk_final_price +
                  "&uid=" +
        ...
        (ret, err) => {
          ...
          if (j + 1 >= that.imgArr.length) {
            ...
            if (api.systemType === "android") {
              this.$utils.useInShare(this.downArr, this.targetApp);
            } else {
              // shareAction 暂不支持安卓targetsdkversion 26, 所以ios中用shareAction
            ...
            if (res.code === 0) {
              if (res.data.rid === "" && res.data.oauth_url !== "") {
                ...
                let img =
                  ...
                  this.$store.state.user.userInfo.uid;
                this.imgArr.push(img);
                if (i + 1 >= this.item.items.length || this.tag == "single") {
                  // 当遍历到最后一张图 或者 tag 是 'single' 时，不需要再往后遍历
                  if (type) {
                    this.save(0);
                  } else {
                    this.shareArr(0);
                  ...
                  .then(response => {
                    if (response.code === 0) {
                      let img =
                        ...
                        this.item.items[i].from_id +
                        "&title=" +
                        encodeURI(res.data[0].title) +
                        "&thumb=" +
                        this.item.items[i].img_url +
                        "&coupon_money=" +
                        res.data[0].coupon_money +
                        "&coupon_price=" +
                        res.data[0].coupon_price +
                        "&origin_price=" +
                        res.data[0].zk_final_price +
                        "&uid=" +
                        this.$store.state.user.userInfo.uid;
                      this.imgArr.push(img);
                      if (i + 1 >= this.item.items.length) {
    ...
    imgShare(targetApp) {
      ...
      if (this.tag == "admin") {
        // 如果tag == 'admin', 就直接分享后端传过来的图片
        this.shareArr(0);
      } else {
        // 如果tag !== 'admin', 就处理下链接，再分享
        this.changeUrl(0);
      }
    },
    imgDown() {
    ...
    articleDownOne() {
      ...
      let img = that.item.video_img
        ? that.item.video_img
        : that.item.article_img;
      let format = "share" + new Date().getTime() + ".jpg";
      api.download(
        {
          url: img,
      ...
      this.$http
        ...
        .then(res => {
          if (res.code === 0) {
            ...
            // console.log(JSON.stringify(this.list.concat(arr)))
