<template>
  <div class="withdraw">
    ...
    <div class="box">
      <!--************************************-->
      <!--余额提现-->
      <div v-if="withdrawIshow" class="head">
      ...
      <div v-else class="head">
        <p>账户收益余额 <span>(元)</span></p>
        <p class="money"><span>¥</span>{{keti || '0.00'}}</p>
      </div>
      <!--**************************************-->
      <div class="high"></div>
      <!--*************************************************-->
      <!--余额提现-->
      <div v-if="withdrawIshow" class="info">
        <div>
          <span>可提现金额（元）</span>
      ...
      <div v-else class="info">
        <div>
          <span>可提现收益（元）</span>
          <span :style="{color: $store.state.global.theme.mainColor}">{{laxin || '0.00'}}</span>
        </div>
        <div>
          <span>提现中收益（元）</span>
          <span :style="{color: $store.state.global.theme.mainColor}">{{tixianzhogn || '0.00'}}</span>
        </div>
      </div>
      <!--***********************************************-->
      <div class="high"></div>
      ...
      <div class="inputBox">
        ...
        <span v-if="withdrawIshow" @click="money = data.credit3" :style="{color: $store.state.global.theme.mainColor}">全部</span>
        <span v-else @click="money = laxin" :style="{color: $store.state.global.theme.mainColor}">全部</span>
      </div>
...
<script>
  ...
  export default {
    ...
    mounted() {
      /*To where the*/
      this.CutString(window.location.href,(d)=>{
        if (d.WithdrawalType == 'PullTheNew') this.withdrawIshow = false;
        this.laxin = ((d.laxin=='undefined')||(d.laxin==null))?'0.00':d.laxin;
        this.keti = ((d.prospect=='undefined')||(d.prospect==null))?'0.00':d.prospect;
      });
      // 删除
      this.getInfo()
    },
    methods: {
      CutString(URL, callback) {
      ...
      widthdraw() {
        ...
        if (this.withdrawIshow) {
          ...
          this.$http.post('/amoy/user/withdraw', {
            ...
            this.$vux.toast.text(res.msg)
            this.data.withdrawal = (Number(this.money) + Number((tixianzhogn || 0))).toFixed(2)
            this.data.credit3 = (this.data.credit3 - this.money).toFixed(2)
            this.info.credit3 = this.data.credit3
            this.$store.commit('setUserInfo', this.info)
            this.money = ''
