    ...
    <div id="box" ref="box">
      ...
      <div class="goodsInst">
        ...
        <div class="priceBox">
          ...
          <div class="profit"  :style="{background: 'linear-gradient('+$store.state.global.theme.deg+'deg,' +$store.state.global.theme.mainColor+','+$store.state.global.theme.secondColor+')'}">预估积分<span style="font-size:0.28rem">{{other.commission_money}}</span>  现金券{{other.commission_money}}</div>
          <!--<div class="profit"  style="background: #9A54E9;">现金券{{other.coupon_price}}</div>-->
        </div>
        <!--<div class="update" v-if="other.level_commission_money && (other.level_commission_money !== other.commission_money) && other.level_name" @click="jumpTo('/rightsConter')">
          <p :style="{color: $store.state.global.theme.mainColor}">现在升级{{other.level_name}}，最高获得{{other.level_commission_money}}佣金</p><span :style="{background: 'linear-gradient('+$store.state.global.theme.deg+'deg,' +$store.state.global.theme.mainColor+','+$store.state.global.theme.secondColor+')'}">立即升级</span>
        </div>-->
...
export default {
  ...
  data () {
    return {
      ...
      save: null,
      getCouponUrl: true,
      couponpassMoney: this.$store.state.user.couponpassMoney ? Number(this.$store.state.user.couponpassMoney) : 0, // 优惠券卡密相关的一个变量
      swiperOption: {
        loop: true,
        on: {
          transitionEnd: () => {
    ...
    oauth () {
      ...
      if (that.$aliPay === 1) {
        ...
        aliBC.asyncInit({
            ...
            //alert('<h1>'+JSON.stringify(ret)+"</h1>")
            //console.log('dayin=='+JSON.stringify(ret));
          if (ret.status) {
            aliBC.showLogin(function (ret, err) {
              if (ret.status) {
                api.openWin({
                  name: 'oauth',
                  url: 'widget://html/oauth.html',
                  reload: true,
                  pageParam: {
        ...
        this.$http.post('/amoy/taobao/goods-item', info, false, true).then(res => {
          if (res.code === 0) {
            if (this.$route.query.way || !this.other.title) {
              ...
              this.$set(this.other, 'level_name', res.data['level_name'])
            }
            this.local(this.other)
            if (this.loading) {
              this.$vux.loading.hide()
      ...
      api.addEventListener({
        ...
        that.$http.post('/amoy/taobao/goods-item', {item_id: that.$route.query.id}, true, true).then(res => {
          if (res.code === 0) {
            ...
            that.$set(that.other, 'level_name', res.data['level_name'])
          }
        })
      })
    }
