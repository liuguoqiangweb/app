      ...
      <div class="lists" >
        <div class="list" v-for="(item, index) of list" :key="index" @click="detail(item)" v-if="index < 9">
          ...
          <div class="estimateBox">
            <span class="estimate" v-if="item.commission_money" style="font-size:0.2rem">预估积分:<span style="font-size:0.23rem">{{item.commission_money}}</span><br/>现金券:{{item.commission_money}}</span>
          </div>
